...
document.getElementById('eventDate').addEventListener('change', function(e) {
    ...
    fetch(`https://planora-catering-3.onrender.com/api/check-availability?date=${date}`)
        .then(res => res.json())
        .then(data => {
            if (data.available) {
...
function updateQuotation() {
    ...
    if (!eventSelect || eventSelect.selectedIndex === -1) return;
...
function confirmPayment() {
    ...
    fetch('https://planora-catering-3.onrender.com/api/create-booking', {
        method: 'POST',
